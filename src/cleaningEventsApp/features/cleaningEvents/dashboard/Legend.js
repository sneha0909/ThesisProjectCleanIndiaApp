...
function Map() {
  ...
  useEffect(() => {
    axios.get("https://localhost:5001/api/Complaints")
      .then((response) => {
    ...
    Promise.all(requests)
      .then((responses) => {
        const childMarkers = responses.map((response, index) => {
          ...
          const marker = L.marker([lat, lng], {complaintStatus: complaints[index].complaintStatus});
        ...
        markers.on('clusterclick', function (event) {
          ...
          const statusList = Object.entries(childStatus).map(([status, count]) => `${status}: ${count}`).join('<br>');
          const popupContent = `<b>Number of Complaints:</b> ${event.layer.getAllChildMarkers().length}<br><br>${statusList}`;
          L.popup()
